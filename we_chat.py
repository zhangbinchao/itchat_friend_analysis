import itchat
from pyecharts import Bar,Pie,Geo,Map,Page,Style
import re
import jieba
...
if __name__ == '__main__':
    ...
    # city = pd.DataFrame({'city': attr, 'url': value})
    # city.to_csv('好友城市.csv', sep=',', header=True, index=True)
    ...
    bar = Bar("我的微信好友分布", "@" + NickName,width=1200)
    bar.add("",   list(attr),list(value),is_label_show=True, is_visualmap=True,)
    # bar.show_config()
    bar.render()
